...
import { ArrowLeft, Phone, Clock, MapPin, ShoppingCart, Plus, Minus, Trash2 } from "lucide-react";
...
const PharmacyDetail = () => {
  ...
  const loadData = async () => {
    const [ph, cats, prods] = await Promise.all([
      supabase.from("pharmacies").select("*").eq("id", pharmacyId).single(),
      supabase.from("pharmacy_categories").select("*").eq("pharmacy_id", pharmacyId).eq("active", true).order("sort_order"),
      supabase.from("pharmacy_products").select("*").eq("pharmacy_id", pharmacyId).eq("active", true).order("name"),
    ]);
    setPharmacy(ph.data);
  ...
  const isOpen = () => {
    if (!pharmacy?.opening_time || !pharmacy?.closing_time) return true;
    const now = new Date();
    if (now.getDay() === 0 && !pharmacy.sunday_open) return false;
    const hhmm = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
    return hhmm >= pharmacy.opening_time && hhmm <= pharmacy.closing_time;
  };
  ...
  const handleCheckout = () => {
    ...
    const pharmacyName = pharmacy?.name || "Farmácia";
    // Navigate to order form with pre-filled data
    localStorage.setItem("pharmacy_order", JSON.stringify({
      description: `Remédio: ${desc}`,
      purchaseLocation: `${pharmacyName} - ${pharmacy?.address || ""}`,
  ...
  if (!pharmacy) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6">
        <p className="text-lg font-bold">Farmácia não encontrada</p>
    ...
    <div className="flex min-h-screen flex-col bg-background pb-24">
      ...
      {pharmacy.banner_url ? (
        <div className="relative h-40 bg-primary/10">
          <img src={pharmacy.banner_url} alt="" className="h-full w-full object-cover" />
      ...
      <div className="px-4 py-4 space-y-3 border-b bg-card">
        <div className="flex items-start gap-3">
          {pharmacy.logo_url ? (
            <img src={pharmacy.logo_url} alt={pharmacy.name} className="h-16 w-16 rounded-xl object-cover border" />
          ) : (
            <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-primary/10 text-3xl">💊</div>
          )}
          <div className="flex-1">
            ...
            {pharmacy.short_description && (
              <p className="text-xs text-muted-foreground mt-0.5">{pharmacy.short_description}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
          {pharmacy.address && (
            <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /> {pharmacy.address}{pharmacy.address_number ? `, ${pharmacy.address_number}` : ""} {pharmacy.neighborhood ? `- ${pharmacy.neighborhood}` : ""}</span>
          )}
          {pharmacy.opening_time && pharmacy.closing_time && (
            <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {pharmacy.opening_time.slice(0, 5)} - {pharmacy.closing_time.slice(0, 5)}</span>
          )}
          {pharmacy.phone && (
            <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {pharmacy.phone}</span>
          )}
        </div>

        {pharmacy.full_description && (
          <p className="text-xs text-muted-foreground">{pharmacy.full_description}</p>
        )}

        <div className="flex gap-2">
          {pharmacy.estimated_delivery_time && (
            <span className="rounded-full bg-secondary px-3 py-1 text-[11px] font-medium">🕐 ~{pharmacy.estimated_delivery_time}min</span>
          )}
          {pharmacy.delivery_fee > 0 && (
            <span className="rounded-full bg-secondary px-3 py-1 text-[11px] font-medium">🛵 R${Number(pharmacy.delivery_fee).toFixed(2)}</span>
          )}
          {pharmacy.accepts_pickup && (
            ...
            return (
              <div key={product.id} className="rounded-xl border bg-card p-3 flex gap-3">
                {product.image_url ? (
                  ...
                  <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-secondary text-xl shrink-0">💊</div>
                )}
